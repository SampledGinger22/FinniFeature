...
// Deterministic seed (spec §13): fixed faker seed → identical dataset each run. Represents
// all six statuses across multiple US states with a has_insurance + email/phone mix, and
// guarantees the headline query ("intake / New York / under 30") returns rows.
...
async function seed(): Promise<void> {
  ...
  for (let statusIndex = 0; statusIndex < STATUSES.length; statusIndex += 1) {
    for (let withinStatus = 0; withinStatus < PATIENTS_PER_STATUS; withinStatus += 1) {
      const record = await createPatient(buildPatientInput(statusIndex, withinStatus));
  ...
  if (archivedId) await archivePatient(archivedId);
  if (deletedId) await softDeletePatient(deletedId);

  console.log(`Seeded ${createdIds.length} patients (1 archived, 1 soft-deleted).`);
  process.exit(0);
}
